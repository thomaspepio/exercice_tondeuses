...
import fr.tpepio.poc.tondeuse.trans.LoggerFacade;
...
public class Launcher {
	...
	/** Message d'erreur utilisé lorsqu'on est incapable de lire la première ligne du fichier. */
	...
	public void doTheJob(String filePath) {
		...
		Grid grid = this.buildGrid(upperRightCase.getAbscisse(), upperRightCase.getOrdonnee());
		...
		for (Mower currentMower : mowers) {
			for (char move : currentMower.getInstructions().toCharArray()) {
				...
				if (EnumMovement.FORWARD.equals(movement)) {
					...
					String message = MessageFormat.format(UNRECOGNIZED_INSTRUCTION, move);
					LOGGER.info(message);
				}
			}

			String movementMessage = MessageFormat.format(MOWER_IS_MOVING, currentMower.getCurrentCase().getAbscisse(), currentMower.getCurrentCase().getOrdonnee(), currentMower.getOrientation().toString());
	...
	/**
	 * Permet de construire une grille dont on suppose que son coin inférieur gauche commence aux coordonnées (0,0), et
	 * dont on fournit les coordonnées du coin suppérieur droit.
	 * 
	 * @param abscisse l'abscisse du coin suppérieur droit.
	 * @param ordonnee l'ordonnée du coin suppérieur droit.
	 ...
	 * 
	 * @param filePath le fichier d'instructions dont la première ligne doit contenir les coordonnées de la case.
	 * @return la case.
	...
	public Case getUpperRightCase(final String filePath) {
		...
		try {
			...
			String message = MessageFormat.format(Launcher.FILE_NOT_FOUND_EXCEPTION, file.getAbsolutePath());
			LoggerFacade.error(this.getClass(), message);
			e.printStackTrace();
		} catch (IOException e) {
			LoggerFacade.error(Launcher.class, Launcher.IO_EXCEPTION_INIT_GRID);
			e.printStackTrace();
		}

		// La ligne est censée contenir les cordonnées de la case séparées par un espace.
	 ...
	 * 
	 * @param theFile le fichier d'entrée.
	 * @param theGrid la grille sur laquelle évolueront les tondeuses.
	 * @return la liste de tondeuses trouvées dans le fichier.
	 * @throws FileNotFoundException levée lorsque le fichier n'existe pas.
	 */
	public List<Mower> buildMowers(final String filePath, final Grid theGrid, ApplicationContext context) {
		...
		try {
			...
			String message = MessageFormat.format(Launcher.FILE_NOT_FOUND_EXCEPTION, filePath);
			LoggerFacade.error(this.getClass(), message);
			e.printStackTrace();
		} catch (IOException e) {
			LoggerFacade.error(this.getClass(), Launcher.IO_EXCEPTION_INIT_MOWERS);
			e.printStackTrace();
		}

		// Récupération de la stratégie contenant le comportement de mouvement des tondeuses.
		...
		for (String currentLine : fileContent) {
			// La première ligne correspond aux coordonnées du coin suppérieur droit de la grille, on en tient pas
			// compte.
			if (i != 1) {
				...
				if (i % 2 == 0) {
					...
					// Sur chaque ligne pair on retrouve les coordonées de départ de la tondeuse et son orientation.
					...
					tempMower.setStrategy(strategy);
					result.add(tempMower);
				} else {
					// Sur chaque ligne impair (exceptée la première) on retrouve la suite d'instructions de la tondeuse.
	...
	/**
	 * Permet de lire un fichier dans son intégralité et le retourne sous forme de liste de String.
	 * 
